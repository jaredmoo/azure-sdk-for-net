    ...
    public class DatabaseCrudScenarioTests
    {
        ...
        public void TestDatabaseTransparentDataEncryptionConfiguration()
        {
            using (SqlManagementTestContext context = new SqlManagementTestContext(this))
            {
                ...
                var config = sqlClient.TransparentDataEncryptionConfigurations.Get(resourceGroup.Name, server.Name, dbName);
                Assert.Equal(TransparentDataEncryptionStatus.Enabled, config.Status);

                // Update TDE config
                config.Status = TransparentDataEncryptionStatus.Disabled;
                config = sqlClient.TransparentDataEncryptionConfigurations.CreateOrUpdate(resourceGroup.Name, server.Name, dbName, config);
